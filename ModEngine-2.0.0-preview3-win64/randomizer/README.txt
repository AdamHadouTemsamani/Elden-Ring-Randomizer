...
**Translators wanted!** If you're a native speaker of languages supported by Elden Ring without existing translations (Italiano, 한국어, Português, Русский, ไทย), or notice partially missing translations, contact me and fill in the files in `diste\Messages` to help make this mod available in other languages. Also let me know if features are needed like font changes or space adjustments. (Better fonts for CJK are coming soon!)
...
This could indicate two things: either `config_eldenring.toml` has not been edited with the right values, or the game has updated and so it no longer can use modded `regulation.bin` files meant for older versions of the game. If you're using a version of the randomizer from at least the latest game patch, then double-check Step 2a to make sure the config is correct.
